      ...
      console.log(new Date()," *** knxgaTodpt : erreur *** ",err);
		...
		if (results.length > 0) {
			if (results[0].knx_ga_datapointtype.length > 0) {
				...
				console.log("*** knxgaTodpt : erreur adresse de groupe connue mais DPT non renseigné")
        console.log(new Date()," *** knxgaTodpt : erreur adresse de groupe connue mais DPT non renseigné");
			}

		} else {
			console.log("*** Fonction knxgaTodpt : erreur adresse de groupe inconnue, mettre à jour table knx_ga de MySQL avec import ETS");
...
var insert_db_knx = function(evt, src, dest, value) {

	knxgaTodpt(dest.toString(), function(result) {
		newdpt = dpstTodpt(result.knx_ga_datapointtype);
		knxmonitorcomment = result.knx_ga_groupname;
		var knxdatapoint = new knx.Datapoint({
			ga: dest.toString(),
			dpt: newdpt
		});

		if (dest.toString() == knxdatapoint.options.ga) {
			knxdatapoint.update(dpts.fromBuffer(value, knxdatapoint.dpt));
			knxmonitorvalue = knxdatapoint.current_value;
			knxmonitorsubtypeunit = knxdatapoint.dpt.subtype.unit;

			//console.log(knxmonitorvalue);
			//console.log(knxmonitorsubtypeunit);

			if (knxdatapoint.dpt.subtype.enc !== undefined) {
				//console.log(knxdatapoint.dpt.subtype.enc[knxdatapoint.current_value]);
				knxmonitorvalue = knxdatapoint.dpt.subtype.enc[knxdatapoint.current_value];
			}
		} else {
			knxmonitorvalue = dest.toString();
		}

////Insertion dans la table knx_monitor
    var sql_knx_monitor = "INSERT INTO knx_monitor (knx_monitor_timestamp, knx_monitor_src, knx_monitor_dest, knx_monitor_value, knx_monitor_comment, knx_monitor_dptsubtypeunit) VALUES ?";
    var timestamp = new Date();
    var values_knx_monitor = [
			[timestamp, src, dest, knxmonitorvalue, knxmonitorcomment, knxmonitorsubtypeunit]
		];
		module_mysql.mysqlconnection.query(sql_knx_monitor, [values_knx_monitor], function(err, result) {
			if (err) {
        console.log("*** mysqlconnection.query : erreur *** ",err);
        console.log(new Date()," *** mysqlconnection.query : erreur *** ",err);
      }
			console.log("*** mysqlconnection : " + result.affectedRows + " records inserted in table knx_monitor *** " + values_knx_monitor);
		});

////Insertion dans la table knx_ga
	  var sql_knx_ga = "UPDATE knx_ga SET knx_ga_timestamp = ?, knx_ga_src = ?, knx_ga_value = ? WHERE knx_ga_address = ?";
    var values_knx_ga = [timestamp, src.toString(), knxmonitorvalue, dest.toString()];
    module_mysql.mysqlconnection.query(sql_knx_ga, values_knx_ga, function(err, result) {
      if (err) {
        console.log("*** mysqlconnection.query : erreur *** ",err);
        console.log(new Date()," *** mysqlconnection.query : erreur *** ",err);
      }
      console.log("*** mysqlconnection : " + result.affectedRows + " records inserted in table knx_ga *** "+ values_knx_ga);
    });
...
json[dest.toString().replace(/\//g, "-")] = knxmonitorvalue;
	...
	headers: headers
        ...
        if (!error && response.statusCode === 200) {
            console.log(body)
				       ...
				       var idinput = JSON.stringify(json[1][destknx].id);   // id de l'input de EMONCMS correspondant à la key (qui est elle égale au GA KNX)
							 ...
							 request(checkdescriptionfield, function (error, response, body) {
								if (!error && response.statusCode === 200) {
								      ...
								      var destmod = dest.toString().replace(/\//g, "-");

											console.log("debug inputdescription knxmonitorcomment " + knxmonitorcomment);
                      ...
                      if(destmod === testarray[i].name){
											 ...
											 var setdescriptionemoncms = {
											 	url: encodeURI('http://' + process.env.EMONCMS_HOST + '/emoncms/input/set.json?inputid=' + testarray[i].id + '&fields={"description":' + '"' + result.knx_ga_groupname + '"' + '}' + '&apikey=' + process.env.EMONCMS_APIKEY),
											 	method: 'GET',
											 	encoding: 'utf8',
											 	headers: headers
											 };
...
exports.insert_db_knx = insert_db_knx;
