...
var insert_emoncms = function(evt, src, dest, value, etsjson) {
	var knxdatapoint = new knx.Datapoint({
		ga: dest.toString(),
		dpt: dpstTodpt(knxgaTodpt(dest.toString(), etsjson))
	});
				...
				if (!error && response.statusCode === 200) {
					...
					request(checkdescriptionfield, function(error, response, body) {
						if (!error && response.statusCode === 200) {
							...
							for (i = 0; i < (testarraylength - 1); i +=1) {
								if (testarray[i].description === "") {
									...
									console.log("knx ga name " + knxgaToganame(dest.toString(), etsjson));
									if (testarray[i] !== undefined && destmod === testarray[i].name) {
										console.log(" ******* OK destmod et testarray name identiques ******");
										setdescriptionemoncms = {
											url: encodeURI('http://' + process.env.EMONCMS_HOST + '/emoncms/input/set.json?inputid=' + testarray[i].id + '&fields={"description":' + '"' + knxgaToganame(dest.toString(),etsjson) + + '"' + '}' + '&apikey=' + process.env.EMONCMS_APIKEY),
											method: "GET",
											encoding: "utf8",
											headers: headers
										};
