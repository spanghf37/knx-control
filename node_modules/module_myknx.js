...
var insert_emoncms = function(evt, src, dest, value) {
	...
	//Configure the request
	var sendemoncms2 = {
		url: encodeURI("http://" + process.env.EMONCMS_HOST2 + "/emoncms/input/post.json?node=1&fulljson=" + JSON.stringify(json) + "&apikey=" + process.env.EMONCMS_APIKEY2),
		method: "GET",
		encoding: "utf8",
		headers: headers
	};
	var getinputemoncms2 = {
		url: "http://" + process.env.EMONCMS_HOST2 + "/emoncms/input/get_inputs.json" + "&apikey=" + process.env.EMONCMS_APIKEY2,
		method: "GET",
		encoding: "utf8",
		headers: headers
	};
	request(sendemoncms2, function(error, response, body) {
		if (!error && response.statusCode === 200) {
			console.log(body);
			request(getinputemoncms2, function(error, response, body) {
				if (!error && response.statusCode === 200) {
					var jsongetinput = JSON.parse(body);
					var destknx = dest.toString().replace(/\//g, "-");
					var idinput = JSON.stringify(jsongetinput[1][destknx].id); // id de l"input de EMONCMS correspondant à la key (qui est elle égale au GA KNX)
					var checkdescriptionfield = {
						url: "http://" + process.env.EMONCMS_HOST2 + "/emoncms/input/list.json" + "&apikey=" + process.env.EMONCMS_APIKEY2,
						method: "GET",
						encoding: "utf8",
						headers: headers
					};
								...
								if (testarray[i].description === "") {
									...
									if (destmod === testarray[i].name) {
										...
										if (knxdatapointunit !== undefined) {
											console.log(" ******* knxdatapointunit : " + knxdatapointunit);
											setdescriptionemoncms = {
												url: encodeURI('http://' + process.env.EMONCMS_HOST2 + '/emoncms/input/set.json?inputid=' + testarray[i].id + '&fields={"description":' + '"' + knxdatapointname + knxdatapointunit + '"' + '}' + '&apikey=' + process.env.EMONCMS_APIKEY2),
												method: "GET",
												encoding: "utf8",
												headers: headers
											};
											...
											setdescriptionemoncms = {
												url: encodeURI('http://' + process.env.EMONCMS_HOST2 + '/emoncms/input/set.json?inputid=' + testarray[i].id + '&fields={"description":' + '"' + knxdatapointname + '"' + '}' + '&apikey=' + process.env.EMONCMS_APIKEY2),
												method: "GET",
												encoding: "utf8",
												headers: headers
											};
