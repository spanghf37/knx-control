					...
					request(checkdescriptionfield, function(error, response, body) {
						if (!error && response.statusCode === 200) {
							...
							for (i = 0; i < testarray.length; i +=1) {
								console.log(" ***** testarray " + JSON.stringify(testarray[74]));
								...
								if (testarray[i].description === "") {
									...
									console.log("knx ga name " + knxgaToganame(dest.toString(), ets));
									console.log("valeur de i " + i);
									if (destmod !== testarray[i].name) {
									console.log("***** testarray " + testarray[i].name);
									};
									if (destmod === testarray[i].name) {
										console.log(" ******* OK destmod et testarray name identiques ******");
										setdescriptionemoncms = {
											url: encodeURI('http://' + process.env.EMONCMS_HOST + '/emoncms/input/set.json?inputid=' + testarray[i].id + '&fields={"description":' + '"' + knxgaToganame(dest.toString(),ets) + + '"' + '}' + '&apikey=' + process.env.EMONCMS_APIKEY),
											method: "GET",
											encoding: "utf8",
											headers: headers
										};
