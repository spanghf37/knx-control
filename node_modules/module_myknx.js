...
var insert_emoncmsbis = function(evt, src, dest, value) {
	//console.log("****** dpstTodpt : " + dpstTodpt(knxgaTodpt(dest.toString(), ets)));
	var knxdatapoint = new knx.Datapoint({
		ga: dest.toString(),
		dpt: dpstTodpt(knxgaTodpt(dest.toString(), ets))
	...
	//Configure the request
	var sendemoncms = {
		url: encodeURI("http://" + process.env.EMONCMS_HOSTBIS + "/emoncms/input/post.json?node=1&fulljson=" + JSON.stringify(json) + "&apikey=" + process.env.EMONCMS_APIKEYBIS),
		method: "GET",
		encoding: "utf8",
		headers: headers
	};
	var getinputemoncms = {
		url: "http://" + process.env.EMONCMS_HOSTBIS + "/emoncms/input/get_inputs.json" + "&apikey=" + process.env.EMONCMS_APIKEYBIS,
		method: "GET",
		encoding: "utf8",
		headers: headers
	};
		...
		if (!error && response.statusCode === 200) {
			...
			request(getinputemoncms, function(error, response, body) {
				if (!error && response.statusCode === 200) {
					...
					var idinput = JSON.stringify(jsongetinput[1][destknx].id); // id de l"input de EMONCMS correspondant à la key (qui est elle égale au GA KNX)
					var checkdescriptionfield = {
						url: "http://" + process.env.EMONCMS_HOSTBIS + "/emoncms/input/list.json" + "&apikey=" + process.env.EMONCMS_APIKEYBIS,
						method: "GET",
						encoding: "utf8",
						headers: headers
					};
								...
								if (testarray[i].description === "") {
									...
									if (destmod === testarray[i].name) {
										...
										if (knxdatapointunit !== undefined) {
											console.log(" ******* knxdatapointunit : " + knxdatapointunit);
											setdescriptionemoncms = {
												url: encodeURI('http://' + process.env.EMONCMS_HOSTBIS + '/emoncms/input/set.json?inputid=' + testarray[i].id + '&fields={"description":' + '"' + knxdatapointname + knxdatapointunit + '"' + '}' + '&apikey=' + process.env.EMONCMS_APIKEYBIS),
												method: "GET",
												encoding: "utf8",
												headers: headers
											};
											...
											setdescriptionemoncms = {
												url: encodeURI('http://' + process.env.EMONCMS_HOSTBIS + '/emoncms/input/set.json?inputid=' + testarray[i].id + '&fields={"description":' + '"' + knxdatapointname + '"' + '}' + '&apikey=' + process.env.EMONCMS_APIKEYBIS),
												method: "GET",
												encoding: "utf8",
												headers: headers
											};
...
exports.insert_emoncmsbis = insert_emoncmsbis;
